#!/bin/bash
#SBATCH --job-name=gp_prod_build
#SBATCH --account=abucsek98
#SBATCH --partition=standard
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=1
#SBATCH --mem=16gb
#SBATCH --time=01:00:00
#SBATCH --output=/scratch/abucsek_root/abucsek0/yfjin/Granular_RubySim_202608/AnalysisScripts/jobs/logs/gp_prod_build_%j.log
#SBATCH --mail-type=END,FAIL
#SBATCH --mail-user=yfjin@umich.edu

set -eo pipefail

SCRIPT_DIR="/scratch/abucsek_root/abucsek0/yfjin/Granular_RubySim_202608/AnalysisScripts"
PIPELINE_DIR="${SCRIPT_DIR}/Pipeline"
TMP_DIR="${SCRIPT_DIR}/jobs/tmp"
PIPE_OUT="/scratch/abucsek_root/abucsek0/yfjin/Granular_RubySim_202608/AnalysisResults/periodic_boundaries/2026-08-03/0_graph_and_basic_stats/artifacts/raw_graph_data"

export PS1="${PS1:-}"
source /home/yfjin/Research/anaconda3/etc/profile.d/conda.sh
conda activate graph_analysis
set -u

export GRAPHPIPE_PROJECT_ROOT="/scratch/abucsek_root/abucsek0/yfjin/Granular_RubySim_202608"
export GRAPHPIPE_OUT_PATH="${PIPE_OUT}"
export GRAPHGEN_GEOMETRY_FILTER="0deg,30deg"
export GRAPHGEN_MAX_SIMS_PER_GEOMETRY=0
export PYTHONUNBUFFERED=1
export MPLCONFIGDIR="${TMP_DIR}/mpl_${SLURM_JOB_ID}"

mkdir -p "${MPLCONFIGDIR}" "${GRAPHPIPE_OUT_PATH}" "${SCRIPT_DIR}/jobs/logs"

cd "${PIPELINE_DIR}"
echo "Production pipeline build started: $(date)"
echo "GRAPHPIPE_OUT_PATH=${GRAPHPIPE_OUT_PATH}"
echo "GRAPHGEN_GEOMETRY_FILTER=${GRAPHGEN_GEOMETRY_FILTER}"
python GraphPipelineBuildGraphs.py
echo "Production pipeline build finished: $(date)"
