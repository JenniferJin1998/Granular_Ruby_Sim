...
def cluster_row(H,cluster_id,G,box_lengths,periodic_axes,cfg,domain_min,domain_max):
    ...
    eigvals,eigvecs=np.linalg.eigh(gyration); order=np.argsort(eigvals)[::-1]; eigvals=np.maximum(eigvals[order],0); eigvecs=eigvecs[:,order]; principal=eigvecs[:,0]; rg=float(np.sqrt(eigvals.sum())); eigenvalue_tolerance=np.finfo(float).eps*max(eigvals[0],1.0)
    # Aspect ratio measures longest/thinnest spread, whereas elongation measures
    # longest/intermediate spread and therefore does not punish planar clusters.
    aspect=float(np.sqrt(eigvals[0]/eigvals[2])) if eigvals[2]>eigenvalue_tolerance else np.nan
    elongation=float(np.sqrt(eigvals[0]/eigvals[1])) if eigvals[1]>eigenvalue_tolerance else np.nan
    ...
    for *_,d in H.edges(data=True):
        angle=d.get("angle_with_zz_periodic",d.get("angle_with_zz",np.nan))
        if np.isfinite(angle): orientations.append(float(np.cos(np.radians(angle))))
    orientation=np.asarray(orientations,float); align=float(abs(principal[loading])); ext=np.ptp(xyz,axis=0)
    return dict(cluster_id=cluster_id,node_count=n,edge_count=e,extent_x=ext[0],extent_y=ext[1],extent_z=ext[2],radius_of_gyration=rg,
                gyration_eigenvalue_1=eigvals[0],gyration_eigenvalue_2=eigvals[1],gyration_eigenvalue_3=eigvals[2],aspect_ratio=aspect,elongation=elongation,
...
def run_task(angle,sim,cfg,root,graphs=None,task_number=None):
    out=root/"job6_force_clusters"/f"{angle}_sim{sim:03d}_clusters.csv"; summary_out=out.with_name(out.stem.replace("_clusters","")+"_simulation_summary.csv"); log=setup_logging(cfg,"job6",task_number)
    ...
    if clusters.empty:
        # Preserve the schema required by completion validation and merging even
        # when a fixed threshold yields no connected high-force components.
        clusters=pd.DataFrame(columns=["cluster_id","node_count"])
    clusters.insert(0,"load_step",cfg["load_step"]); clusters.insert(0,"sim_idx",sim); clusters.insert(0,"angle",angle)
    ...
    atomic_csv(out,clusters); atomic_csv(summary_out,summary); mark_complete(out,cfg,{"angle":angle,"sim_idx":sim,"clusters":len(clusters),"periodic_axes":periodic_axes}); log.info("Complete %s",out)


def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--config",default=CONFIG_PATH); ap.add_argument("--task-id",type=int); ap.add_argument("--all",action="store_true",help="Run every simulation while loading the graph pickle only once"); ap.add_argument("--dry-run",action="store_true"); args=ap.parse_args(); cfg=load_config(args.config); root=ensure_layout(cfg); graphs=load_graph_dict(cfg); all_tasks=tasks(graphs,cfg)
    if args.all:
        if args.dry_run: print(f"Would run {len(all_tasks)} force-cluster tasks"); return
        for tid,(angle,sim) in enumerate(all_tasks): run_task(angle,sim,cfg,root,graphs=graphs,task_number=tid)
        return
    ...
    angle,sim=all_tasks[tid]
    if args.dry_run: print(tid,angle,sim); return
    run_task(angle,sim,cfg,root,graphs=graphs,task_number=tid)
